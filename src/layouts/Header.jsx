...
import { FiLogOut, FiSearch, FiMenu,FiHome, FiX, FiUser } from "react-icons/fi";
...
const Header = () => {
  ...
  return (
    <header className="z-40 relative w-full px-4 py-2 bg-black text-white">
      <div className="container mx-auto flex justify-between items-center">
        <Link to="/" className="flex items-center">
          <img src={NETFLIX_LOGO} alt="Netflix" className="w-24 md:w-32" />
        </Link>
      ...
      >
        {languages.map((language) => (
          ...
          <NavItems user={user} lang={lang} handleSignOut={handleSignOut} handleLanguageChange={handleLanguageChange} />
        </nav>
      </div>
      ...
      {isMenuOpen && (
        <nav className="md:hidden mt-2 py-2 bg-black">
          ...
          <NavItems user={user} lang={lang} handleSignOut={handleSignOut} handleLanguageChange={handleLanguageChange} isMobile />
        </nav>
      )}
    </header>
  );
...
const NavItems = ({ user, lang, handleSignOut, handleLanguageChange, isMobile }) => {
  ...
  return (
    <div className='flex flex-col md:flex-row items-start md:gap-4'>
      ...
      <Link to="/gpt" className={`${buttonClass} md:bg-purple-600 md:hover:bg-purple-700 `}>
        <FiSearch className="mr-2 text-base " />
        {languageText.headerText[lang].gptSearch}
      </Link>
      {user ? (
          <button onClick={handleSignOut} className={`${buttonClass} md:bg-gray-600 md:hover:bg-gray-700 flex`}>
