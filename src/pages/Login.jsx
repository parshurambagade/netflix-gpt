        ...
        <form className="flex md:mt-4 lg:mt-20 text-white w-full min-w-96 max-w-96 bg-transparent lg:bg-black  rounded-lg  md:w-1/2 lg:w-3/12 p-6 md:p-2 lg:p-12  lg:bg-opacity-85 mx-auto  md:flex flex-col gap-4 lg:box-border md:text-left  text-sm ">
          <h2 className="font-bold text-2xl lg:text-3xl lg:my-4">
            {isSignInForm
              ...
              : languageText?.loginPage[lang]?.signUp}
          </h2>
          {!isSignInForm && (
            <input
              type="text"
              ...
              className="w-full bg-gray-800 p-4 border border-gray-500 rounded-md"
            />
          )}
          <input
            ref={email}
            type="email"
            name="email"
            placeholder={languageText?.loginPage[lang]?.emailAddress}
            className="w-full bg-gray-800 p-4 border border-gray-500 rounded-md"
          />
          <input
            ref={password}
            type="password"
            name="password"
            placeholder={languageText?.loginPage[lang]?.enterPassword}
            className="w-full bg-gray-800 p-4 border border-gray-500 rounded-md"
          />
          ...
          <button
            type="submit"
            className="bg-red-700 font-bold p-4  w-full rounded-md"
          ...
          <p className="md:text-base  lg:text-sm">
            {languageText?.loginPage[lang]?.forgotPassword}
          </p>
          ...
          <div className="my-4 sm:my-8  md:my-16 lg:my-8 lg:mx-0">
            {isSignInForm ? (
              ...
              <p className="text-white ">
                {languageText?.loginPage[lang]?.alreadyRegistered}{" "}
                <span
                  className="font-bold cursor-pointer"
                  onClick={() => setIsSignInForm(!isSignInForm)}
                >
