...
const GptSearchPage = () => {
  const { movies, loading, error, movieNames } = useSelector(
    (state) => state.gpt
  );
      ...
      <main className="absolute z-10 flex justify-center w-full py-8 lg:py-[12vh] gap-20 flex-col">
        ...
        <section aria-label="GPT Recommended Movies">
          {loading === true ? (
            ...
            <SearchResults movieNames={movieNames} movies={movies} />
          )}
        </section>
      </main>
    </div>
...
const SearchResults = ({ movieNames, movies }) => {
  ...
  return (
    <div
      className={`bg-black bg-opacity-70 py-4 lg:py-8 px-4 mx-2 -my-16 sm:-my-8 lg:mx-20`}
    >
